...
export const lyricsRoute = async (app) => {
    app.get('/lyrics', async (request, reply) => {
        ...
        if (!q)
            return reply.code(400).send({ error: 'Provide artist or title' });
        try {
            async function fetchLrclib(query) {
                const resp = await fetch(`https://lrclib.net/api/search?q=${encodeURIComponent(query)}`, { headers: { 'User-Agent': 'lyrics-app/1.0' } });
                if (!resp.ok)
                    throw new Error(`LRCLIB responded ${resp.status}`);
            ...
            if (!results.length && artist && title) {
                ...
                if (firstArtist !== artist)
                    results = await fetchLrclib(`${firstArtist} ${title}`);
            }
            if (!results.length) {
