...
<img src="docs/demo.gif" alt="Demo: searching for a track, synced lyrics arriving with pronunciation and translation, tapping a word for its definition" width="100%">

<br><br>

<img src="docs/stats-banner.svg" alt="Project stats: demo songs, starter dictionary words, live lyrics sources — generated by scripts/gen_stats_banner.py">
...
**Language scope, stated plainly:** this is a Russian-learning tool today — the
pronunciation engine, bundled dictionary, and UI copy are all Russian-specific.
Ukrainian and other languages are on the roadmap, not shipped; nothing here
claims otherwise.
...
- **Tap a word → definition.** Lemma-aware lookup (`гаснет` → `гаснуть`), part of speech, gloss, usage note. The card stays open while the song keeps playing, and dismisses with the ✕, a tap outside, or <kbd>Esc</kbd>.
- **Scroll back anytime.** The lyric panel follows the active line, but scrolling up to reread pauses that — it resumes once you stop, or on a tap of "resume following."
...
- **Six background presets**, switchable live: Audio waves (2D equalizer, the default), **Ocean** — WebGL, four summed wave trains split into bass/mid/treble bands so swell, ripple and specular sparkle each answer to a different part of the spectrum — Field lines, Refraction, Fracture, and **Neural network** — a constellation of nodes that light up and connect as the track gets louder and busier, each node permanently tracking one frequency bin.
- All of it driven by an `AnalyserNode` (with a musical synthetic fallback when the stream is cross-origin), plus wave height, reactivity and blur sliders, and four colour themes — Штиль, Прибой, Ночь, Лагуна.
- **Eco mode** drops WebGL entirely for a lightweight CSS/Canvas2D stand-in.
...
│   ├── bg/ocean.ts           three.js wave shader (bass/mid/treble reactive)
│   ├── bg/neural.ts          Canvas2D audio-reactive node network
...
└── scripts                   README asset generators — demo GIF, stats banner
```
...
```bash
npx playwright test e2e/record-demo.spec.ts   # records a fresh walkthrough video
./scripts/make-demo-gif.sh                    # → docs/demo.gif (ffmpeg, two-pass palette)
python3 scripts/gen_stats_banner.py            # → docs/stats-banner.svg, read live from server/src/data
```

The stats banner has no dependencies beyond `python3` and reads the demo catalogue
and dictionary source directly — re-run it after adding songs or glossary entries so
